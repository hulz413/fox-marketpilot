...
定义 MarketPilot 内部研究质量就绪检查、演示就绪状态、RAG 索引与检索评测摘要、生成内容 smoke check、前端内部展示和安全边界契约，用于帮助团队在演示前判断已完成研究任务是否可复查、可演示。
...
系统 SHALL 将研究质量就绪状态保持为内部能力，帮助团队准备演示，同时避免把内部评测暴露给用户进度页和公开报告读者。

#### Scenario: 任务进度页不展示演示就绪检查

- **WHEN** 用户打开一条已完成研究任务的进度页
- **THEN** 前端不读取 latest readiness run
- **AND** 页面不展示「演示就绪检查」面板、运行检查入口、检查项摘要或 RAG 检索评测关联提示
- **AND** 页面继续展示研究状态、查看研究结果入口和阶段时间线

#### Scenario: 前端保留 readiness API client

- **WHEN** 需要在内部工具或后续页面接入 readiness 能力
- **THEN** 前端 API client 保留创建 readiness run 和读取 latest readiness run 的函数
- **AND** 这些函数不被研究进度页默认调用
- **AND** readiness 检查失败不影响商机推荐、报告和来源入口
