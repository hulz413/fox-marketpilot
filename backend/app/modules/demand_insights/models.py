from __future__ import annotations

import uuid as uuid_pkg
from datetime import datetime, timezone
from typing import Optional

from sqlalchemy import DateTime, ForeignKey, Index, JSON, String, Uuid
from sqlalchemy.orm import Mapped, mapped_column

from app.db.base import Base


def utc_now() -> datetime:
    return datetime.now(timezone.utc)


class OpportunityDemandInsight(Base):
    __tablename__ = "opportunity_demand_insights"
    __table_args__ = (
        Index(
            "ix_demand_insights_task_deleted_at",
            "research_task_id",
            "deleted_at",
        ),
        Index(
            "ix_demand_insights_opportunity_deleted_at",
            "opportunity_id",
            "deleted_at",
        ),
        Index("ix_demand_insights_deleted_at", "deleted_at"),
    )

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    uuid: Mapped[uuid_pkg.UUID] = mapped_column(
        Uuid(as_uuid=True),
        unique=True,
        nullable=False,
        default=uuid_pkg.uuid4,
    )
    research_task_id: Mapped[int] = mapped_column(
        ForeignKey("research_tasks.id"),
        nullable=False,
    )
    opportunity_id: Mapped[int] = mapped_column(
        ForeignKey("opportunities.id"),
        nullable=False,
    )
    summary: Mapped[str] = mapped_column(String(1200), nullable=False)
    audience_profile: Mapped[str] = mapped_column(String(800), nullable=False)
    use_cases: Mapped[list[str]] = mapped_column(JSON, nullable=False, default=list)
    purchase_motivations: Mapped[list[str]] = mapped_column(
        JSON,
        nullable=False,
        default=list,
    )
    content_angles: Mapped[list[str]] = mapped_column(
        JSON,
        nullable=False,
        default=list,
    )
    trend_signals: Mapped[list[str]] = mapped_column(
        JSON,
        nullable=False,
        default=list,
    )
    seasonality_notes: Mapped[str] = mapped_column(String(800), nullable=False)
    source_status: Mapped[str] = mapped_column(String(32), nullable=False)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        nullable=False,
        default=utc_now,
    )
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        nullable=False,
        default=utc_now,
        onupdate=utc_now,
    )
    deleted_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
    )


class OpportunityDemandInsightSource(Base):
    __tablename__ = "opportunity_demand_insight_sources"
    __table_args__ = (
        Index(
            "ix_demand_insight_sources_insight_deleted_at",
            "demand_insight_id",
            "deleted_at",
        ),
        Index(
            "ix_demand_insight_sources_source_deleted_at",
            "research_source_id",
            "deleted_at",
        ),
        Index("ix_demand_insight_sources_deleted_at", "deleted_at"),
    )

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    uuid: Mapped[uuid_pkg.UUID] = mapped_column(
        Uuid(as_uuid=True),
        unique=True,
        nullable=False,
        default=uuid_pkg.uuid4,
    )
    demand_insight_id: Mapped[int] = mapped_column(
        ForeignKey("opportunity_demand_insights.id"),
        nullable=False,
    )
    research_source_id: Mapped[int] = mapped_column(
        ForeignKey("research_sources.id"),
        nullable=False,
    )
    relevance_note: Mapped[str] = mapped_column(String(1000), nullable=False)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        nullable=False,
        default=utc_now,
    )
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        nullable=False,
        default=utc_now,
        onupdate=utc_now,
    )
    deleted_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
    )
