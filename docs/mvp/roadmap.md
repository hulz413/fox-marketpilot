...
| P2       | 演示体验增强。用于分享、导出、历史和内部演示质量检查等不阻塞核心演示流程的能力。                         |

## 当前进度

截至 2026-06-09，`bootstrap-project-skeleton`、`bootstrap-product-skeleton`、`create-research-task`、`run-opportunity-research`、`observe-agent-runs`、`show-research-progress`、`polish-ui-ux`、`rank-opportunity-recommendations`、`view-opportunity-detail`、`generate-final-report`、`start-demo-research`、`collect-research-sources`、`show-source-transparency`、`explain-demand-insights`、`introduce-rag-retrieval`、`suggest-supply-candidates`、`compare-competitor-references`、`estimate-validation-budget`、`review-opportunity-risks`、`create-action-plan`、`evaluate-rag-quality`、`browse-research-history`、`share-final-report`、`add-research-quality-readiness`、`streamline-research-navigation`、`add-generation-quality-evaluation`、`add-internal-quality-panel`、`add-research-intake-conversations` 和 `parallelize-research-analysis` 已完成实现，`review-opportunity-risks` 已归档到 `openspec/changes/archive/2026-05-31-review-opportunity-risks/`，`create-action-plan` 已归档到 `openspec/changes/archive/2026-05-31-create-action-plan/`，`introduce-rag-retrieval` 已归档到 `openspec/changes/archive/2026-05-31-introduce-rag-retrieval/`，`evaluate-rag-quality` 已归档到 `openspec/changes/archive/2026-05-31-evaluate-rag-quality/`，`share-final-report` 已归档到 `openspec/changes/archive/2026-05-31-share-final-report/`，`add-research-quality-readiness` 已归档到 `openspec/changes/archive/2026-06-01-add-research-quality-readiness/`，`streamline-research-navigation` 已归档到 `openspec/changes/archive/2026-06-07-streamline-research-navigation/`，`add-generation-quality-evaluation` 已归档到 `openspec/changes/archive/2026-06-07-add-generation-quality-evaluation/`，`add-internal-quality-panel` 已归档到 `openspec/changes/archive/2026-06-07-add-internal-quality-panel/`，`add-research-intake-conversations` 已归档到 `openspec/changes/archive/2026-06-07-add-research-intake-conversations/`，`parallelize-research-analysis` 已归档到 `openspec/changes/archive/2026-06-08-parallelize-research-analysis/`，`project-skeleton`、`product-skeleton`、`research-tasks`、`opportunity-research`、`opportunity-results`、`agent-run-observability`、`research-progress`、`research-sources`、`demand-insights`、`rag-retrieval`、`rag-quality-evaluation`、`supply-candidates`、`competitor-references`、`validation-budgets`、`opportunity-risks`、`action-plans`、`report-sharing`、`research-quality-readiness`、`generation-quality-evaluation`、`internal-quality-panel` 与 `research-intake-conversations` 主规格已同步。当前已经跑通“中文示例一键启动、聊天式需求对齐或自定义表单输入 → 更新结构化研究草稿 → 确认创建任务 → 进入任务进度页 → 查看当前运行阶段 → 失败后重新运行 → 完成后进入商机列表/详情/基础报告 → 保存后收集公开来源线索 → 整理任务内 RAG 证据索引 → 并行生成需求洞察、货源候选和基于任务内检索证据的竞品参考 → 汇总研究发现 → 生成利润与验证预算粗算 → 生成商机风险复核 → 生成行动计划与询盘话术 → 生成报告只读分享链接 → 通过在线分享页浏览报告快照 → 从演示用户菜单进入内部质量复查面板 → 运行内部生成质量评测 → 运行内部研究质量就绪检查 → 查看 RAG 质量摘要 → 通过来源、RAG 检索、RAG 检索质量评测、需求洞察、货源候选、竞品参考、验证预算、风险复核、行动计划、生成质量评测、研究质量就绪检查、内部质量面板和需求对齐会话 API 读取 → 查看 Agent trace 与阶段事件”的闭环，并补齐了统一我的研究入口、任务上下文导航、聊天式新建研究、需求草稿确认、移动端卡片列表、真实研究历史兼容入口、无任务上下文结果入口引导、可行动空状态、轻量来源透明度 UI、需求洞察展示、任务内 RAG evidence chunks 与 retriever、中文 RAG 检索评测集、P0 检索指标、货源候选展示、竞品参考展示、验证预算展示、风险复核展示、行动计划展示、报告在线分享、研究质量就绪检查、RAG 索引健康摘要、RAG 检索评测摘要、生成质量评测集、rubric 指标摘要、内部质量面板和并行分析阶段观测；基础商机生成阶段仍直接基于任务输入生成结构化商机，不在生成推荐前做外部前置调研、RAG 或竞品深挖。下一步可以在后续独立切片中补充演示数据治理、评测集扩展或内部访问保护。

说明：`browse-research-history` 未单独建立归档 change；该切片由 `bootstrap-product-skeleton` 的历史入口骨架、`polish-ui-ux` 的真实任务历史入口打磨和 `streamline-research-navigation` 的统一研究列表共同覆盖，需求已沉淀在 `product-skeleton` 与 `research-tasks` 主规格中。
...
| `browse-research-history`          | complete | P2       | 用户能回看之前完成或失败的商机研究任务，方便继续查看演示案例和历史结果。             | 提供研究历史列表，展示任务标题、创建时间、状态和入口；用户可以打开已完成任务的报告，或重新运行失败/历史任务。该切片由 `bootstrap-product-skeleton` 和 `polish-ui-ux` 共同覆盖，不另设独立归档 change。                         | 研究历史列表。                   | 用户能看到历史研究任务；能打开已完成任务；能识别任务状态；能从历史任务进入报告或重新运行入口。                                 |
| `streamline-research-navigation`   | complete | P2       | 用户能从统一入口查找、继续和阅读研究结果，不在任务、历史、商机和报告入口之间迷路。       | 将全局导航收束为新建研究和我的研究；用统一研究列表承载历史、状态筛选、进度、重新运行和已完成任务结果入口；缺少任务上下文的商机推荐和报告入口只展示选择研究或新建研究引导。                              | 统一研究列表；任务上下文结果导航。         | 一级导航不再并列展示研究历史、商机推荐和最终报告；`/history` 保留兼容入口；已完成任务主操作为查看研究结果；无任务上下文结果入口不自动选择最近任务或展示静态 demo。 |
...
| `add-research-quality-readiness`   | complete | P2       | 团队能在演示前快速判断一条完成任务是否可演示，以及哪些研究节点需要复查。             | 为已完成研究任务运行内部质量就绪检查，汇总阶段完整性、RAG 索引健康、RAG 检索评测摘要、生成内容 smoke check 和分享快照状态；保留内部 API 和安全摘要供内部工具读取；公开分享页和用户侧进度页不展示内部检查。 | readiness run；内部演示状态；质量检查摘要。 | 已完成任务可运行 readiness 检查；未完成任务不会被标记为可演示；检查失败不修改研究主流程状态或结果；响应不暴露内部 ID、chunk 原文、trace 细节或敏感配置。 |
| `add-generation-quality-evaluation` | complete | P2       | 团队能在演示前复跑已完成研究结果，发现输入约束、结构、语义和谨慎边界问题。           | 建立中文生成质量评测集；保存 generation evaluation run 和逐 case result；检查预算、渠道、排除品类、目标人群、供给偏好、字段完整性、增强一致性、风险质量、行动质量和谨慎边界；将摘要接入 readiness。 | 生成质量评测集；evaluation run；rubric 摘要。 | 已完成任务可运行生成质量评测；case 失败不终止整次评测；readiness 可读取最近评测摘要；响应不暴露内部 ID、完整 prompt、完整网页正文或原始异常堆栈。 |
| `add-internal-quality-panel`       | complete | P2       | 团队能在演示前从一个内部页面集中复查已完成研究任务的质量状态。                   | 新增 `/internal/quality` 内部质量面板；从演示用户菜单进入；支持选择已完成任务、触发 readiness run 和 generation evaluation run，并展示 readiness、生成质量评测和 RAG 质量摘要；一级产品导航、研究进度页、报告页和公开分享页不暴露内部质量信息。 | 内部质量复查面板；质量运行入口；RAG 摘要。 | 已完成任务可从面板运行质量复查；面板展示最近质量结果和安全错误摘要；未完成任务不会被标记为可演示；响应和 UI 不暴露内部 ID、完整 prompt、完整网页正文、RAG chunk 原文或原始异常堆栈。 |
| `add-research-intake-conversations` | complete | P2       | 用户在想法还模糊时，也能通过聊天把商机需求整理成可执行研究任务。                 | 新增研究需求对齐会话、消息保存、结构化草稿更新、完整度判断、默认假设、确认创建研究任务和首页聊天式新建研究入口；确认后复用现有研究任务创建和启动流程，不在聊天阶段生成商机结果、来源、RAG 或报告。 | 聊天式需求对齐会话；结构化研究草稿；确认启动入口。 | 用户能通过自然语言聊天补充需求；点击更新需求后看到结构化草稿、缺失条件和默认假设；信息足够时可确认创建并启动真实研究任务；API 不暴露内部 ID、完整 prompt、模型原始输出或异常堆栈。 |
| `parallelize-research-analysis`    | complete | P2       | 用户能更快看到增强分析进展，团队也能在 trace 中看清多个专业分析分支如何协作。        | 将需求洞察、货源候选和竞品参考调整为来源与 RAG 之后并行执行的 specialist analysis branches，并在预算、风险和行动计划前增加研究发现汇总边界；不新增用户输入字段、公开结果 API 或外部依赖。 | 并行分析编排；研究发现汇总阶段；分支级观测。 | 来源和 RAG 后能并行启动需求、货源和竞品分支；任一分支失败不会覆盖基础商机结果或阻止其他分支完成；进度页、阶段事件和 trace metadata 能展示分支状态、汇总结果和安全失败摘要。 |
