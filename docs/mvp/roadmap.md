...
截至 2026-06-07，`bootstrap-project-skeleton`、`bootstrap-product-skeleton`、`create-research-task`、`run-opportunity-research`、`observe-agent-runs`、`show-research-progress`、`polish-ui-ux`、`rank-opportunity-recommendations`、`view-opportunity-detail`、`generate-final-report`、`start-demo-research`、`collect-research-sources`、`show-source-transparency`、`explain-demand-insights`、`introduce-rag-retrieval`、`suggest-supply-candidates`、`compare-competitor-references`、`estimate-validation-budget`、`review-opportunity-risks`、`create-action-plan`、`evaluate-rag-quality`、`browse-research-history`、`share-final-report`、`add-research-quality-readiness`、`streamline-research-navigation` 和 `add-generation-quality-evaluation` 已完成实现，`review-opportunity-risks` 已归档到 `openspec/changes/archive/2026-05-31-review-opportunity-risks/`，`create-action-plan` 已归档到 `openspec/changes/archive/2026-05-31-create-action-plan/`，`introduce-rag-retrieval` 已归档到 `openspec/changes/archive/2026-05-31-introduce-rag-retrieval/`，`evaluate-rag-quality` 已归档到 `openspec/changes/archive/2026-05-31-evaluate-rag-quality/`，`share-final-report` 已归档到 `openspec/changes/archive/2026-05-31-share-final-report/`，`add-research-quality-readiness` 已归档到 `openspec/changes/archive/2026-06-01-add-research-quality-readiness/`，`streamline-research-navigation` 已归档到 `openspec/changes/archive/2026-06-07-streamline-research-navigation/`，`add-generation-quality-evaluation` 已归档到 `openspec/changes/archive/2026-06-07-add-generation-quality-evaluation/`，`project-skeleton`、`product-skeleton`、`research-tasks`、`opportunity-research`、`opportunity-results`、`agent-run-observability`、`research-progress`、`research-sources`、`demand-insights`、`rag-retrieval`、`rag-quality-evaluation`、`supply-candidates`、`competitor-references`、`validation-budgets`、`opportunity-risks`、`action-plans`、`report-sharing`、`research-quality-readiness` 与 `generation-quality-evaluation` 主规格已同步。当前已经跑通“中文示例一键启动或自定义输入 → 创建任务 → 进入任务进度页 → 查看当前运行阶段 → 失败后重新运行 → 完成后进入商机列表/详情/基础报告 → 保存后收集公开来源线索 → 整理任务内 RAG 证据索引 → 生成需求洞察 → 生成货源候选 → 基于任务内检索证据生成竞品参考 → 生成利润与验证预算粗算 → 生成商机风险复核 → 生成行动计划与询盘话术 → 生成报告只读分享链接 → 通过在线分享页浏览报告快照 → 运行内部生成质量评测 → 运行内部研究质量就绪检查 → 在我的研究列表查看演示状态 → 通过来源、RAG 检索、RAG 检索质量评测、需求洞察、货源候选、竞品参考、验证预算、风险复核、行动计划、生成质量评测和研究质量就绪 API 或内部命令读取 → 查看 Agent trace 与阶段事件”的闭环，并补齐了统一我的研究入口、任务上下文导航、移动端卡片列表、真实研究历史兼容入口、无任务上下文结果入口引导、可行动空状态、轻量来源透明度 UI、需求洞察展示、任务内 RAG evidence chunks 与 retriever、中文 RAG 检索评测集、P0 检索指标、货源候选展示、竞品参考展示、验证预算展示、风险复核展示、行动计划展示、报告在线分享、研究质量就绪检查、RAG 索引健康摘要、RAG 检索评测摘要、生成质量评测集、rubric 指标摘要和演示状态 badge；基础商机生成阶段仍直接基于任务输入生成结构化商机，不在生成推荐前做外部前置调研、RAG、竞品深挖或多 Agent 协作。下一步可以在后续独立切片中补充演示数据治理、评测集扩展或内部质量面板。
...
| `add-research-quality-readiness`   | complete | P2       | 团队能在演示前快速判断一条完成任务是否可演示，以及哪些研究节点需要复查。             | 为已完成研究任务运行内部质量就绪检查，汇总阶段完整性、RAG 索引健康、RAG 检索评测摘要、生成内容 smoke check 和分享快照状态；在进度页展示内部「演示就绪检查」面板，并在历史列表展示轻量状态 badge；公开分享页不展示内部检查。 | readiness run；内部演示状态；质量检查摘要。 | 已完成任务可运行 readiness 检查；未完成任务不会被标记为可演示；检查失败不修改研究主流程状态或结果；响应不暴露内部 ID、chunk 原文、trace 细节或敏感配置。 |
